<template>
    <el-drawer :visible.sync="open_" :direction="direction" :with-header="withHeader" :size="size"
               class="project-plugin-drawer" :projectId="projectId" :isAdmin="isAdmin">
        ...
        <div v-if="isAdmin">
            <el-button type="primary" size="small" style="margin-right:32px;" class="fr"
                       @click="openCreatedPlugin">添加</el-button>
        </div>
        <div style="background:#fafafa;padding:12px;" v-if="pluginSaveOpen">
            ...
            <div style="text-align: center; margin-top: 10px;">
                <el-button type="primary" plain size="small" @click="executeSaveProjectPlugin('pluginForms')">提交
                </el-button>
                <el-button plain size="small" @click="cancelSaveProjectPlugin('pluginForms')">取消</el-button>
            </div>
        </div>
        <div style="padding:0 32px;margin-top:24px;">
            <el-table :data="projectPlugins">
                <el-table-column type="expand">
                    <template slot-scope="projectPlugin">
                        <el-form label-position="left" class="table-expand" size="small">
                            <el-form-item v-for="(configVal, configKey) in projectPlugin.row.config"
                                          :label="configKey">
                                <span>{{ configVal }}</span>
                            </el-form-item>
                        </el-form>
                ...
                <el-table-column label="操作" width="160px">
                    <template slot-scope="projectPlugin">
                        <el-button type="primary" size="mini" @click="openUpdatedPlugin(projectPlugin.row)"
                                   :disabled="!isAdmin">编辑
                        </el-button>
                        <el-popconfirm
                                ...
                                icon="el-icon-info"
                                iconColor="red"
                                @onConfirm="executeRemoveProjectPlugin(projectPlugin.row, 'pluginForms')"
    ...
    export default {
        ...
        watch: {
            open(value) {
                if (value) {
                    if (this.projectId > 0) {
                        this.updatedProjectAndSystemPlugins();
                    } else {
                        this.cancelPlugin();
                    }
                }
        ...
        computed: {
            open_: {
                ...
                set(val) {
                    ...
                    this.$emit("changePluginProject", val);
                }
            }
        },
        methods: {
            ...
            getProjectPlugins(projectId) {
                this.$oakRequest.get(this.$oakBackendApis.project.plugins(projectId)).then(res => {
                    if (res.status === 200) {
                        let plugins = res.data.plugins;
                        if (Object.keys(plugins).length > 0) {
                            this.projectPlugins = plugins;
                        } else {
                            this.projectPlugins = [];
                        }
                    }
            ...
            cancelSaveProjectPlugin(formName) {
                // updated project and system plugins
                this.updatedProjectAndSystemPlugins();
                // reset form filed
            ...
            executeSaveProjectPlugin(formName) {
                this.$refs[formName].validate(valid => {
                    ...
                    if (this.pluginSaveAction === "UPDATED") {
                        this.$oakRequest.put(this.$oakBackendApis.project.updatedPlugin(this.projectId, plugin.id), plugin).then(res => {
                            if (res.status === 200) {
                                this.$message({
                                    type: "success",
                                    message: "编辑插件成功"
                                ...
                                // close edit area
                                this.cancelSaveProjectPlugin(formName);
                            }
                        });
                    }

                    if (this.pluginSaveAction === "CREATED") {
                        this.$oakRequest.post(this.$oakBackendApis.project.createdPlugin(this.projectId), plugin).then(res => {
                            if (res.status === 200) {
                                this.$message({
                                    type: "success",
                                    message: "添加插件成功"
            ...
            executeRemoveProjectPlugin(plugin, formName) {
                this.$oakRequest.delete(this.$oakBackendApis.project.deletedPlugin(this.projectId, plugin.id)).then(res => {
                    if (res.status === 200) {
                        this.$message({
                            type: "success",
                            message: "删除插件成功"
            ...
            openCreatedPlugin() {
                let projectPluginCurrentNames = [];
                this.projectPlugins.forEach(ele => {
                    projectPluginCurrentNames.push(ele.name);
                });

                this.systemPlugins = this.systemPlugins.filter(ele => {
                    return projectPluginCurrentNames.includes(ele.name) === false;
                });
            ...
            updatedProjectAndSystemPlugins() {
                this.getProjectPlugins(this.projectId);
